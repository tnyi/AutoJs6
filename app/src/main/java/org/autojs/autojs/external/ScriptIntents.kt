...
object ScriptIntents {
    ...
    const val EXTRA_KEY_CONFIG_TAG_WEBSOCKET_URL = "config_tag_websocket_url"
    ...
    fun handleIntent(context: Context?, intent: Intent) {
        ...
        ExecutionConfig().let { config ->
            config.apply {
                ...
                workingDirectory = WorkingDirectoryUtils.path
                setArgument("intent", intent)
                
                intent.getStringExtra(EXTRA_KEY_CONFIG_TAG_WEBSOCKET_URL)?.let { url ->
                    putTag(ScriptExecutionGlobalListener.ENGINE_TAG_WEBSOCKET_URL, url)
                }
            }
